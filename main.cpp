...
int main() 
{
	...
	for (int i = 0; i < imus.size(); i++) {
		...
		//{vxg, vyg, vzg};
		std::vector<double> rot_angles = count_rot_angles(elements[i].t, imus[i].wx, imus[i].wy, imus[i].wz, wgs[0], wgs[1], wgs[2], elements[i].thdg, elements[i].pitch, elements[i].roll);
		// {gamma, psi, theta};
